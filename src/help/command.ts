...
import { HelpOptions } from '@oclif/plugin-help'
...
export default class CommandHelp {
    ...
    generate(): string {
        ...
        // FIX: Remove flag sorting
        const flags = Object.entries(cmd.flags || {}) // sortBy(Object.entries(cmd.flags || {})
            .filter(([, v]) => !v.hidden)
            .map(([k, v]) => {
                v.name = k
                return v
    ...
    protected flags(flags: Config.Command.Flag[]): string | undefined {
        ...
        const body = renderList(flags.map(flag => {
            ...
            if (flag.type === 'option' && flag.default) {
                // FIX: move default to right
                right = `${right} [default: ${flag.default}]`
            }
